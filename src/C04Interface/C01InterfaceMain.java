package C04Interface;

import java.util.ArrayList;
import java.util.LinkedList;
import java.util.List;
import java.util.Queue;

public class C01InterfaceMain {
    public static void main(String[] args) {
        C01Cat c1 = new C01Cat();
        c1.makeSound();

        C01Dog d1 = new C01Dog();
        d1.makeSound();

//      인터페이스 특징1 - 다형성 : 하나의 부모타입으로 여러 자식객체를 다룰 수 있음.
//      타입(인터페이스)은 같고 구현체는 다름
        C01AnimalInterface1 c2 = new C01Cat();
        c2.makeSound();
        C01AnimalInterface1 d2 = new C01Dog();
        d2.makeSound();

//        <나의 정리>
//      인터페이스 타입으로 참조하면
//      사용할 수 있는 메소드는 인터페이스에 선언된 것만이다.
//      하지만 실행은 구현 클래스의 메소드가 된다.

//        다형성의 장점 : 기존의 구현체(ex-ArrayList)를 다른 구현체(ex-LinkedList)로 변경시 용이.
//        구현체에 의존하지 않고, 코드변경 최소화
//        List<Integer> myList = new ArrayList<>();
        List<Integer> myList = new LinkedList<>();
        myList.add(10);
        myList.add(20);
        myList.get(0);

//      인터페이스 특징2 - 다중구현(상속) 가능
        C01AnimalInterface1 c3 = new C01Cat();
        c3.makeSound();
        C01AnimalInterface2 c4 = new C01Cat();
        c4.eat("츄르");

//        다중구현의 예시 - List와 Queue   [(선언) 객체 = (구현)]
        List<Integer> myList1 = new LinkedList<>();
//        myList1.get(); //사용가능
        Queue<Integer> myList2 = new LinkedList<>();
//        myList2.get(); //사용불가


//            LinkedList(a,b,c,...) -> List, deque
//            deque(a,b,c) -> Queue (a,b)
//            이중에서 LinkedList가 구현체가 가장 많은 것을 의미

    }
}
