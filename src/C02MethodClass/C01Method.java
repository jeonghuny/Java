...
//        같은 클래스 내에서 정의된 클래스메서드의 호출은 클래스명 생략 가능
        ...
        Collections.sort(myList); // Collections는 클래스다. sort는 static으로 선언
    }
//    접근제어자(public), 클래스메서드(static)/객체메서드(static X), 반환타입(int, String, void 등), 매개변수(input값)

    public static int getTotal(int start, int end){
        // public : 접근제어자
        // static 붙이면 클래스 메서드, 안 붙이면 객체 메서드
        // int : 반환 타입
        int total=0;
